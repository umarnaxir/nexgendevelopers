      ...
      <div className="md:hidden -mx-4 px-4 mb-6">
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          {pricingOptions.map((option, index) => (
            <div
              key={index}
              className={`glass-card text-white rounded-2xl p-4 sm:p-6 min-h-[300px] sm:min-h-[340px] flex flex-col ${
                index === 2 ? "col-span-2" : ""
              }`}
            >
              <h4 className="text-base sm:text-lg font-bold text-white mb-1 sm:mb-2">{option.title}</h4>
              <p className="text-silver text-xs sm:text-sm mb-3 sm:mb-4">{option.description}</p>
              <ul className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-silver-light mt-auto">
        ...
        <div className="text-center mt-6 px-2">
          <p className="text-sm text-silver mb-2">
            <strong className="text-white">Custom Quotes Available:</strong> Every project is unique. Contact us for a tailored quote.
          </p>
        ...
        <div className="glass p-6 sm:p-8 rounded-2xl">
          ...
          <div className="grid md:grid-cols-3 gap-4 sm:gap-6 mb-4 sm:mb-6">
            {pricingOptions.map((option, index) => (
              ...
              >
                <h4 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-3">{option.title}</h4>
                <p className="text-silver text-sm mb-3 sm:mb-4">{option.description}</p>
          ...
          <div className="text-center pt-2">
            <p className="text-base sm:text-lg text-silver mb-3 sm:mb-4 px-2">
              <strong className="text-white">Custom Quotes Available:</strong> Every project is unique, and we provide personalized pricing based on your specific requirements, timeline, and complexity.
            </p>
            <p className="text-sm sm:text-base text-silver-dark px-2">
