    ...
    <section id="services" className="relative overflow-hidden py-14 sm:py-16 lg:py-20" data-aos="fade-up">
      ...
      <div className="container relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-10 text-center" data-aos="zoom-in">
          {/* decorative icons */}
          <span className="absolute left-0 top-6 hidden h-14 w-14 items-center justify-center rounded-full border border-teal-400/20 bg-teal-400/10 text-teal-300 shadow-lg shadow-teal-500/20 backdrop-blur lg:flex">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm text-silver sm:text-base">
        ...
        {/* ===== Stats strip ===== */}
        <div className="glass mx-auto mb-12 grid max-w-4xl grid-cols-2 gap-y-6 rounded-2xl px-4 py-5 sm:grid-cols-4 sm:divide-x sm:divide-white/[0.06]">
          {stats.map(({ icon: Icon, value, label }) => (
            <div key={label} className="group flex items-center justify-center gap-3 sm:px-2">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-teal-400/20 bg-teal-400/10 text-teal-300 transition-transform duration-300 group-hover:scale-110">
                <Icon className="h-5 w-5" />
              </span>
              <div>
                <p className="text-xl font-extrabold text-white">{value}</p>
                <p className="text-[11px] text-silver-dark sm:text-xs">{label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="relative">
          ...
          <div className="mb-4 flex items-center justify-end gap-2">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-silver-light backdrop-blur transition-all hover:border-teal-400/40 hover:bg-teal-400/10 hover:text-teal-300 active:scale-95"
            >
          ...
          >
            {carouselServices.map((service, index) => {
              ...
              return (
                ...
                >
                  {/* number badge */}
                  <span className="absolute right-5 top-5 text-xs font-bold text-silver-dark/60">{number}</span>
                  {/* faint watermark */}
                  <Icon className="pointer-events-none absolute -right-4 top-6 h-24 w-24 text-white/[0.04] transition-transform duration-500 group-hover:scale-110" />

                  {/* icon tile */}
                  <span className="flex h-12 w-12 items-center justify-center rounded-xl border border-teal-400/20 bg-teal-400/10 text-teal-300 transition-all duration-300 group-hover:scale-110 group-hover:bg-teal-400/20">
        ...
        {/* ===== Bottom CTA bar ===== */}
        <div className="relative mt-12 overflow-hidden rounded-3xl border border-teal-400/15 bg-gradient-to-br from-teal-600/15 via-white/[0.02] to-black p-6 sm:p-8" data-aos="fade-up">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className="text-lg font-bold text-white sm:text-xl">Have a project in mind?</p>
            ...
            <button
              ...
              className="group inline-flex shrink-0 items-center gap-2.5 self-start rounded-full border border-white/15 bg-white/[0.06] py-1.5 pl-6 pr-1.5 text-sm font-bold text-white backdrop-blur transition-all duration-300 hover:border-teal-400/50 hover:bg-white/[0.1] hover:scale-105 active:scale-95 lg:self-auto"
            >
              Let&apos;s Talk
          ...
          <div className="relative mt-6 flex flex-wrap gap-x-8 gap-y-2 border-t border-white/[0.08] pt-5">
            {ctaFeatures.map((feature) => (
              <span key={feature} className="inline-flex items-center gap-2 text-sm font-medium text-silver-light">
